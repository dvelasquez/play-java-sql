...
public class DataConnectionFactory implements IDataActions {
    ...
    public DataConnectionFactory() {
        ...
        if (Configuration.root().keys().contains("nazca.replication.enable")) {
            if (Configuration.root().getBoolean("nazca.replication.enable")) {

                String replicationNodes = Configuration.root().getString("nazca.replication.nodes");
...
//            throw new Exception(3300, parameter.getName(), ex.getMessage());
    ...
    @Override
    public EntityRepository ExecuteQuery(final DataService service) throws Exception {
        ...
        EntityRepository rep = new DefaultEntityRepository((ArrayList<ResultSet>) result);
    ...
    @Override
    public EntityRepository ExecuteSql(final DataService service) throws Exception {
        ...
        try {
            ...
            EntityRepository rep = new DefaultEntityRepository(resultsets);
